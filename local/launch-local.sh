...
MAUTRIX_SIGNAL_IMAGE_TAG="${MAUTRIX_SIGNAL_IMAGE_TAG:-v0.2604.0}"
...
REPEATER_SERVICE_AS_TOKEN="${REPEATER_SERVICE_AS_TOKEN:-}"
REPEATER_SERVICE_HS_TOKEN="${REPEATER_SERVICE_HS_TOKEN:-}"
...
prepare_repeater_service() {
  ...
  if [[ -z "${REPEATER_SERVICE_AS_TOKEN}" ]]; then
    REPEATER_SERVICE_AS_TOKEN="$(random_hex_token)"
  fi
  if [[ -z "${REPEATER_SERVICE_HS_TOKEN}" ]]; then
    REPEATER_SERVICE_HS_TOKEN="$(random_hex_token)"
  fi
